...
// Mismo orden que usa /board — pendientes y en revisión primero, completadas
// al final, tanto para las columnas del Kanban como para las secciones de la
// vista de lista.
const STATUSES: TaskStatus[] = ['pending', 'in-progress', 'review', 'scheduled', 'blocked', 'done']
...
export function ProjectDetail({ project, onEdit }: Props) {
  ...
  const openNewTask = (status: TaskStatus = 'pending') => { setEditingTask(null); setNewTaskStatus(status); setTaskModalOpen(true) }
  ...
  const handleKanbanDrop = (taskId: string, targetStatus: TaskStatus) => {
      ...
      <div className="flex flex-col gap-5">
        ...
        >
            ...
            <div className="flex items-center gap-2">
              ...
              >
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-md transition-colors"
                  style={{
                    backgroundColor: tasksView === 'kanban' ? 'var(--tp-dark)' : 'transparent',
                    color: tasksView === 'kanban' ? 'var(--tp-lime)' : 'var(--tp-text-2)',
                  }}
                >
          ...
          {projectTasks.length === 0 ? (
            ...
            <div className="flex gap-3.5 overflow-x-auto pb-2" style={{ height: '600px' }}>
              {STATUSES.map((status) => (
                <KanbanColumn
                  key={status}
                  status={status}
                  ...
                  onDrop={(taskId) => handleKanbanDrop(taskId, status)}
                />
              ))}
            </div>
          ) : (
            <div className="flex flex-col gap-3">
              {STATUSES.filter((status) => tasksByStatus[status]?.length).map((status) => {
